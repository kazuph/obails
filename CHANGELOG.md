...
- サイドバー境界のresize handleをオーバーレイ化し、テーマ色が透ける縦帯を除去する
- Explorerのフォルダ行へ、非表示対象を除いた再帰的なMarkdownノート数を表示する
- 最後のタブを閉じた分割ペインを削除し、不要なEmpty paneを残さない
- タブ上端のクリップと本文上端の隙間を除去し、水平スクロール中もタブ下端を本文罫線へ接触させる
- 隣接タブ間を2px空け、スクロールバーがタブ行の高さを占有しないようにする
